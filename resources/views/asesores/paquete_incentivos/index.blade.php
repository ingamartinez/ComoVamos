    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box" id="hide">
                ...
                <form id="form-agregar-linea" method="POST" autocomplete="off">
                    {{csrf_field()}}
                    <div class="row-">
                        {{--<div class="col-lg-6">--}}
                    ...
                    <div class="form-group text-left m-b-0">
                        <button class="btn btn-success waves-effect waves-light" type="submit" id="#validar">
                            Validar
                        </button>
                ...
                <table id="datatable" class="table table-striped table-bordered">
                    ...
                    @foreach($paquetes as $paquete)
                        <tr data-id="{{$paquete->id}}" class="{{($paquete->trashed() ? 'danger': false)}}">
                            <td>{{$paquete->cedula}}</td>
                            <td>{{$paquete->name}}</td>
                            <td>{{$paquete->sucursal->name}}</td>

                            <td>{{$paquete->updated_at->format('D d, M Y - h:i:sa')}}</td>
                            <td>{{$paquete->created_at->format('D d, M Y - h:i:sa')}}</td>
